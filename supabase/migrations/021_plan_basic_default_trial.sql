-- Plan básico por defecto: 1 liña, 5 usuarios, gratuíto o primeiro mes.

alter table public.organizations
  add column if not exists trial_ends_at timestamptz;

comment on column public.organizations.trial_ends_at is 'Para plan basic: fin do período de proba gratuíto (created_at + 1 mes). Null en plans de pago.';

alter table public.organizations
  alter column plan_id set default 'basic';

-- Crear organización con plan basic, 1 liña, trial 1 mes
...
begin
  ...
  insert into public.organizations (name, plan_id, created_by, organization_code, trial_ends_at)
  values (name_trimmed, 'basic', uid, code_trimmed, now() + interval '1 month')
  ...
  values (org_id, 'Línea A');

  return org_id;
end;
$$;

-- Unirse a organización: plan basic ata 5 usuarios
...
begin
  ...
  if member_count >= max_members then
    raise exception 'La organización ha alcanzado el límite de usuarios de su plan. Pide al administrador que actualice el plan.';
  end if;
